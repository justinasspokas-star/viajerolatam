(function(){
  ...
  document.addEventListener('DOMContentLoaded', function(){ var consent = getStoredConsent(); if(consent && consent.analytics === true){ gtag('consent', 'update', {'analytics_storage': 'granted'}); loadGA(); showSettingsButton(); } else if(consent && consent.analytics === false){ clearAnalyticsCookies(); showSettingsButton(); } else { showBanner(); } });
  window.vlCookieConsent = { open: openSettings, acceptAll: function(){ setConsent(true); }, rejectNonEssential: function(){ setConsent(false); }, reset: function(){ try{localStorage.removeItem(STORAGE_KEY);}catch(e){}; clearAnalyticsCookies(); location.reload(); } };
})();
